...
describe('SkipServerSideRender Component', () => {
  test('should only render the server-side pieces', async () => {
    ...
    expect(res.text).toEqual(expect.stringContaining('SkipServerSideRender/server'));
    expect(res.text).not.toEqual(expect.stringContaining('SkipServerSideRender/client'));
  });
});
